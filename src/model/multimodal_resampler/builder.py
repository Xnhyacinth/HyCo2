...
from src.model.multimodal_resampler.sampler import Resampler, ResamplerWithText
...
class TextGuidedRouterAttention(nn.Module):
    ...
    def forward(self, x, text, attn_mask=None):
        ...
        out, weight = self.cross_attn(x, text, text, key_padding_mask=~attn_mask)
...
class TextGuidedRouterCosine(nn.Module):
    ...
    def forward(self, image, text_embedding, attn_mask=None):
        ...
        similarity_matrix = torch.nn.functional.cosine_similarity(
            image.unsqueeze(1), text_embedding.unsqueeze(0), dim=-1
        )

        if attn_mask is not None:
            attn_mask = attn_mask == False
            similarity_matrix = similarity_matrix.masked_fill(
                attn_mask.unsqueeze(0), float(0)
            )
        ...
        return (
            similarity_matrix  # softmax_with_temperature(similarity_matrix, self.temp)
        )
...
class TextGuidedSampler(nn.Module):
    ...
    def __init__(self, projector_type, config):
        ...
        if projector_type == "cosine":
            self.selector = TextGuidedRouterCosine(
