...
class RobertaforConfig(RobertaConfig):
    model_type = "robertafor"
    ...
    def forward(
        ...
        if token_type_ids is None:
            if hasattr(self, "token_type_ids"):
                ...
                # try:
                # print("token_type_ids", buffered_token_type_ids.shape)
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention, position_embedding_type=None):
        super().__init__()
    ...
    def __init__(self, config, layer_idx):
        ...
        if self.add_cross_attention and layer_idx >= config.cross_attn_start_layer:
            ...
            self.crossattention = RobertaAttention(
                config, is_cross_attention=True, position_embedding_type="absolute"
            )
...
class RobertaEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [RobertaLayer(config, i) for i in range(config.num_hidden_layers)]
        )
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    (
                        ...
                        if encoder_hidden_states is not None
                        and isinstance(encoder_hidden_states, list)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            if (
                ...
                and len(encoder_hidden_states) <= self.config.num_hidden_layers
            ):
                # padding None to encoder_hidden_states
                encoder_hidden_states = [None] * (
                    self.config.num_hidden_layers - len(encoder_hidden_states)
                ) + encoder_hidden_states
