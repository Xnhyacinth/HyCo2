...
def keyword_extraction_with_tfidf(documents, topk=1):
    ...
    ret = []
    for doc_index, doc in tqdm(enumerate(documents), desc="TF-IDF"):
        doc_tfidf_scores = tfidf_matrix.toarray()[doc_index]
        keywords_with_scores = {
            feature_names[col]: doc_tfidf_scores[col]
            for col in range(len(feature_names))
        }
        top_keywords = sorted(
            keywords_with_scores.items(), key=lambda item: item[1], reverse=True
        )[:topk]
...
def calculate_information_loss(original_text, compressed_text):
    original_entropy = calculate_entropy(original_text)
    compressed_entropy = calculate_entropy(compressed_text)
    
    information_loss = original_entropy - compressed_entropy
    return information_loss


def calculate_rouge_scores(original_text, compressed_text):
    rouge = Rouge()
    scores = rouge.get_scores(compressed_text, original_text)
    return scores[0]['rouge-l']['f']


def evaluate_bertscore(original_text, compressed_text):
    # original_texts = [original_text]
    # compressed_texts = [compressed_text]
    
    P, R, F1 = score(compressed_text, original_text, lang="en", verbose=True)
