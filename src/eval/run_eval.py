...
def load_dataset(data, use_rag, args):
    ...
    dev_data = None
    # test_path = f"compress/data/eval/{data}/test.jsonl"
    # if data == "2wikimqa":
    #     test_path = f"compress/data/eval/{data}/dev.jsonl"
...
if __name__ == "__main__":
    ...
    if "answers" in test_data[0].keys():
        answers = [x["answers"] for x in test_data]
    else:
    ...
    if args.reconstruct:
        ...
        readability_score, information_loss, rouge_score = 0, 0, 0
        num_pred = len(generated_results)
        for pred, ctx in zip(generated_results, context):
            readability_score += calculate_readability_score(pred)
            information_loss += calculate_information_loss(ctx, pred)
            rouge_score += calculate_rouge_scores(ctx, pred)

        bertscore = evaluate_bertscore(context, generated_results)

    elif args.eval_metrics == "substring_match":
        ...
        score = f"{f1}-{rl}"
        score_per_sample = [
            (f1_score, rl_score) for f1_score, rl_score in zip(f1_scores, rl_scores)
        ]
    if args.reconstruct:
        result_dict = {
            "dataset": args.data,
            ...
            "bertscore": bertscore,
            "readability_score": readability_score / num_pred,
            "information_loss": information_loss / num_pred,
            "rouge_score": rouge_score / num_pred
        }
    else:
        result_dict = {
            "dataset": args.data,
