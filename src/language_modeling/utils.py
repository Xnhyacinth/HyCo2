...
def get_kl_loss(
    ...
    student_mask = (
        (student_labels != -100).unsqueeze(-1).expand_as(student_logits)
    )  ## batch_size,num_tokens,vocab_size
    student_logits_selected = torch.masked_select(student_logits, student_mask).view(
        -1, vocab_size
    )

    teacher_mask = (teacher_labels != -100).unsqueeze(-1).expand_as(teacher_logits)
    teacher_logits_selected = torch.masked_select(teacher_logits, teacher_mask).view(
        -1, vocab_size
    )

    if distill_topk is not None:
        _, topk_teacher_indices = torch.topk(
            teacher_logits_selected, k=distill_topk, dim=-1
        )

        teacher_logits_selected = torch.gather(
            teacher_logits_selected, 1, topk_teacher_indices
        )
        student_logits_selected = torch.gather(
            student_logits_selected, 1, topk_teacher_indices
        )

    assert (
        teacher_logits_selected.shape == student_logits_selected.shape
...
def encode_with_messages_format(example, tokenizer, max_seq_length):
    ...
    tokenized_example = tokenizer(
        example_text, max_length=max_seq_length, truncation=True
    )
...
def save_with_accelerate(
    ...
    if save_projector_only:
        ...
        # breakpoint()
        if isinstance(unwrapped_model, peft.peft_model.PeftModelForCausalLM):
...
XInstructions = [
    "Using the background {xrag_token}, generate a logical and coherent continuation paragraph.",
    "Consider the background {xrag_token}. Write the next paragraph that fits the context.",
    "Based on the background  {xrag_token}, draft a suitable continuation paragraph.",
    "Referencing the background {xrag_token}, create a seamless continuation.",
    "Incorporate the background {xrag_token} to generate the next segment of the text.",
    "Leverage the background {xrag_token} to produce the next logical section.",
    "Using {xrag_token} as the background, write the next paragraph.",
    "Generate a follow-up paragraph that incorporates the background {xrag_token}.",
    "From the given background {xrag_token}, create a continuation paragraph.",
    "Background: {xrag_token}\n",
    "To provide accurate answers, it's essential to consider the background information presented here. Contextual Background: {xrag_token}\n",
    "Background: {xrag_token}\nYou might find the above background documents helpful.\n",
    "The following background will help you understand the context for the questions. Please read it carefully before responding. Background: {xrag_token}\n",
]
...
def find_matched_index(main_seq, sub_seq):
    ...
    assert (
        len(sub_seq) > 0 and len(main_seq) > 0
