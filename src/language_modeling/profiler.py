...
from src.model import XMistralConfig, XMistralForCausalLM
...
if __name__ == "__main__":
    ...
    config = XMistralConfig.from_pretrained(
        pretrained_model_name_or_path, retriever_hidden_size=retriever_hidden_size
    )
    model = (
        XMistralForCausalLM.from_pretrained(
            pretrained_model_name_or_path, config=config, torch_dtype=torch_dtype
        )
        .to(device)
        .eval()
    )
    ...
    num_added_tokens = tokenizer.add_tokens(
        [AddedToken(XRAG_TOKEN, lstrip=False, rstrip=False)]
    )
    ...
    retrieval_kwargs = {}
    if "gated" in pretrained_model_name_or_path:
        input_ids = torch.randint(
            low=0, high=vocab_size - 1, size=(batch_size, instruction_length + 16)
        ).to(device)
        attention_mask = torch.ones_like(input_ids)
        input_ids[:, 3 : 3 + 16] = xrag_token_id
        retrieval_kwargs["retrieval_embeds"] = torch.rand(
            num_docs * batch_size, retriever_hidden_size, dtype=torch_dtype
        ).to(device)

    elif use_xrag:
        input_ids = torch.randint(
            low=0, high=vocab_size - 1, size=(batch_size, instruction_length + num_docs)
        ).to(device)
        attention_mask = torch.ones_like(input_ids)
        ...
        input_ids = torch.randint(
            low=0,
            high=vocab_size - 1,
            size=(batch_size, instruction_length + document_length),
        ).to(device)
        attention_mask = torch.ones_like(input_ids)
    ...
    total_cuda_time = model_inference_event.cuda_time / 1000**2 / num_trails
