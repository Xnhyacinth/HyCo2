...
from .utils import XRAG_TOKEN, ParaphraseInstructions, RAGInstructions, Instructions, XInstructions


def split_background(
    background, tokenizer, total_max_len, single_max_len, single_min_len=20
):
    """
    ...
    """
    ids = tokenizer(
        background, add_special_tokens=False, max_length=total_max_len, truncation=True
    ).input_ids
    background = [
        ids[idx : idx + single_max_len] for idx in range(0, len(ids), single_max_len)
    ]
...
def _concat_messages_llama(messages, tokenizer):
    ## llama Chat Format
    return tokenizer.apply_chat_template(
                messages, tokenize=False
            )

def _concat_messages_qwen(messages, tokenizer):
    ## qwen Chat Format
    return tokenizer.apply_chat_template(
                messages, tokenize=False
            )
        ...
        if message["role"] != "assistant":
            if message_idx == 0:
                ...
                message_start_idx = tokenizer(
                    _concat_messages(messages[:message_idx], tokenizer),
                    return_tensors="pt",
                    max_length=max_seq_length,
                    truncation=True,
                ).input_ids.shape[1]

            if chat_format in ["mistral", "mixtral", "llama", "qwen"]:
...
def encode_with_chat_format_pretrain(
    ...
    #     print(new_tokenizer)
    #     print(tokenizer)
    if "raw_content" in example and example["raw_content"] is not None:
        document = example["raw_content"].strip()
    else:
        document = example["text"].strip()
    answer = document
    if "cformer" in projector_type:
        xrag_token = "".join([XRAG_TOKEN] * retrieval_embed_length).strip()
    elif "prompt" in projector_type:
        xrag_token = XRAG_TOKEN
    elif "gated" in projector_type:
        if "cat" in projector_type:
            xrag_token = "".join([XRAG_TOKEN] * retrieval_embed_length * 2).strip()
        else:
            xrag_token = "".join([XRAG_TOKEN] * retrieval_embed_length).strip()
    elif "mlp2x" in projector_type:
        xrag_token = " ".join([XRAG_TOKEN] * retrieval_embed_length)
    elif "selectp" in projector_type:
        xrag_token = XRAG_TOKEN

    if 'mypt' in projector_type:    
        docs = document.split(' ')
        # split_index = int(0.9 * len(docs))
        if "raw_content" in example and example["raw_content"] is not None and len(docs) > 450:
            split_index = random.randint(5, 100)
        else:
            split_index = random.randint(5, 45)
        # if split_index > len(docs) and len(docs) > 3:
        #     split_index = min(5, len(docs) - 3)
        # elif split_index > len(docs) and len(docs) <= 3:
        #     split_index = 1
        # if len(docs) == 0:
        #     breakpoint()
        document, answer = ' '.join(docs[:-split_index]), ' '.join(docs[-split_index:])
        # if 'mix' in projector_type:
        #     instruction = random.choice(Instructions).format_map(
        #         dict(background=xrag_token)
        #     )
        # else:
        instruction = random.choice(XInstructions).format_map(
            dict(xrag_token=xrag_token)
        )
    elif 'mix' in projector_type:
        idx = random.randint(0, 25)
        docs = document.split(' ')
        len_docs = len(docs)
        if idx <= 12:
            if len_docs <= 80:
                idx = random.randint(13, 25)
            else:
                # split_index = int(0.9 * len(docs))
                if "raw_content" in example and example["raw_content"] is not None and len_docs > 450:
                    split_index = random.randint(5, 100)
                else:
                    split_index = random.randint(5, 45)
                document, answer = ' '.join(docs[:-split_index]), ' '.join(docs[-split_index:])
            ...
            if len_docs > 200:
                ...
                if "raw_content" in example and example["raw_content"] is not None and len_docs > 450:
                    ...
                    split_index = random.randint(5, 45)
                document, answer = ' '.join(docs[:-split_index]), ' '.join(docs[-split_index:])
        instruction = Instructions[idx].format_map(
                dict(xrag_token=xrag_token)
            )
    else:
    ...
    ret = {}
    if "cformer" in projector_type and not with_xrag:
        ret["cformer_input_ids"] = retriever_tokenizer(
            instruction,
            return_attention_mask=False,
            add_special_tokens=False,
            return_tensors="pt",
            max_length=512 - num_query_tokens,
            truncation=True,
        ).input_ids.flatten()
    elif "cformer" in projector_type:
        ret["cformer_input_ids"] = _encode_chat_format(
            messages,
            retriever_tokenizer,
            512 - num_query_tokens,
            chat_format=chat_format,
        )["input_ids"]
    elif ("gated" in projector_type or "selectp" in projector_type) and not with_xrag:
        ret["cformer_input_ids"] = retriever_tokenizer(
            instruction,
            return_attention_mask=False,
            add_special_tokens=False,
    ...
    elif "gated" in projector_type or "selectp" in projector_type:
        ret["cformer_input_ids"] = _encode_chat_format(
            messages, retriever_tokenizer, max_seq_length, chat_format=chat_format
        )["input_ids"]

    if 'adagen' in projector_type:
        # num_chunks = (ret["cformer_input_ids"] // 100) + 1
        # num_chunks = len(ret["cformer_input_ids"].split(100, dim=0)) + 1
        xrag_token = "".join([XRAG_TOKEN] * 2).strip()
        if 'mypt' in projector_type:    
            instruction = random.choice(RAGInstructions).format_map(
                    dict(background=xrag_token)
                )
        else:
            instruction = random.choice(ParaphraseInstructions).format_map(
    ...
    # if cformer:
    #     if not with_xrag:
    #         ret['cformer_input_ids'] = retriever_tokenizer(instruction, return_attention_mask=False, add_special_tokens=False, return_tensors='pt', max_length=512 - num_query_tokens, truncation=True).input_ids.flatten()
    #     else:
    #         ret['cformer_input_ids'] = _encode_chat_format(messages,retriever_tokenizer,512 - num_query_tokens,chat_format=chat_format)['input_ids']
    if num_query_tokens != 0:
        ret["retriever_input_text"] = document
...
def encode_with_chat_format_finetune(
    ...
    for idx in range(len(_messages)):
        if _messages[idx]["role"] == "user":
            _messages[idx]["content"] = (
                f"Refer to the background document: {background}\n\n"
    ...
    ## Gated
    if "top10" in projector_type and "top10" in example.keys():
        background = "\n".join([hit["text"] for hit in example["top10"]])

    if use_rag_tuning and use_retriever_embed:
        sharded_background = split_background(
            background,
            retriever_tokenizer,
            total_max_len=max_seq_length,
            single_max_len=180,
        )
        num_split = len(sharded_background)
        ret["retriever_input_text"] = sharded_background
    elif use_rag_tuning and ("cformer" in projector_type or "gated" in projector_type or "selectp" in projector_type):
    ...
    if use_rag_tuning:
        ...
        _messages = copy.deepcopy(messages)
        if "cformer" in projector_type:
            xrag_tokens = "".join([XRAG_TOKEN] * retrieval_embed_length).strip()
        elif "prompt" in projector_type:
            xrag_tokens = XRAG_TOKEN
        elif "gated" in projector_type:
            if "cat" in projector_type:
                xrag_tokens = "".join([XRAG_TOKEN] * retrieval_embed_length * 2).strip()
            else:
                xrag_tokens = "".join([XRAG_TOKEN] * retrieval_embed_length).strip()
        elif "selectp" in projector_type:
            xrag_tokens = XRAG_TOKEN
        ...
        if 'adagen' in projector_type:
            # num_chunks = (ret["cformer_input_ids"] // 100) + 1
            # num_chunks = len(ret["cformer_input_ids"].split(100, dim=0)) + 1
            xrag_tokens = "".join([XRAG_TOKEN] * 2).strip()

        for idx in range(len(_messages)):
            if _messages[idx]["role"] == "user":
                _messages[idx]["content"] = (
                    f"Refer to the background document: {xrag_tokens}\n\n"
                    + messages[idx]["content"]
                )
                if (
                    "cformer" in projector_type or "gated" in projector_type  or "selectp" in projector_type
                ) and not with_xrag:
                    questions.append(messages[idx]["content"])
                break
        ...
        if "cformer" in projector_type and not with_xrag:
            ...
            ret["cformer_input_ids"] = _encode_chat_format(
                _messages, retriever_tokenizer, max_seq_length, chat_format=chat_format
            )["input_ids"]
            # tokenized_target = self.encoder_tokenizer(label, return_attention_mask=False, add_special_tokens=False)
            # encoded = _encode_chat_format(_messages,retriever_tokenizer,max_seq_length,chat_format=chat_format)
            # ret['cformer_input_ids'] = encoded['input_ids']
            # ret['cformer_labels'] = encoded['labels']
    ...
    def get_input_and_labels(prompt, label, background=None):
        ...
        if background is not None:
            ...
            if background_start_idx != -1:
                labels[
                    background_start_idx : background_start_idx + len(background_ids)
                ] = input_ids[
                    background_start_idx : background_start_idx + len(background_ids)
                ]
        ...
        if label_start_idx != -1:  ## extreme long propmt
            labels[label_start_idx : label_start_idx + len(label_ids)] = input_ids[
                label_start_idx : label_start_idx + len(label_ids)
            ]
            labels[-1] = input_ids[-1]  ## eos

        return torch.tensor(input_ids), torch.tensor(labels)

    question, answer, task_type = (
        example["question"].strip(),
        example["answer"].strip(),
        example["task_type"].strip(),
    )
    ...
    if use_rag_tuning:
        ...
        if use_paraphrase_finetune:
            ...
            prompt = (
                ...
                + "\n\n"
                + template.format_map(
                    dict(question=question, answer=answer, background=prompt_background)
                )
            )
        ...
        prompt = start_prompt + template.format_map(
...
def encode_with_completion_format_pretrain(
    example, tokenizer, max_seq_length, retrieval_embed_length, xrag_token_id
):
    document = example["text"].strip()
    ...
    input_ids = (
        input_ids[:first_eos_index] + input_ids[first_eos_index + 1 :]
    )  ## strip the additional eos
...
def encode_with_completion_format_finetune(
    ...
    def get_input_and_labels(prompt, completion):
        ...
        input_ids = tokenized_example.input_ids
        labels = input_ids.clone()
        tokenized_prompt_length = tokenizer(
            prompt, max_length=max_seq_length, truncation=True, return_length=True
        ).length[0]
    ...
    if use_rag_tuning:
        ...
        for idx, prompt_background in enumerate(
            ...
            rag_instruction = random.choice(RAGInstructions).format_map(
                {"background": prompt_background}
            )
...
PARAPHRASE_RAG_FECT_CHECKING_PROPMT = (
    "Background: {background}\nThe above background document is just a paraphrase of the following: {real_background}\n\n"
    + FECT_CHECKING_PROPMT
)
...
PARAPHRASE_RAG_MULTIPLE_CHOICE_PROMPT = (
    "Background: {background}\nThe above background document is just a paraphrase of the following: {real_background}\n\n"
    + MULTIPLE_CHOICE_PROMPT
)
...
PROMPT_TEMPLATES = {
    "open_qa": {
        True: {True: PARAPHRASE_RAG_QA_PROMPT, False: RAG_QA_PROMPT},
        False: QA_PROMPT,
    },
    "fact_checking": {
        True: {
            True: PARAPHRASE_RAG_FECT_CHECKING_PROPMT,
            False: RAG_FECT_CHECKING_PROPMT,
        },
        False: FECT_CHECKING_PROPMT,
    },
    "multiple_choice": {
        True: {
            True: PARAPHRASE_RAG_MULTIPLE_CHOICE_PROMPT,
            False: RAG_MULTIPLE_CHOICE_PROMPT,
        },
        False: MULTIPLE_CHOICE_PROMPT,
    },
}
