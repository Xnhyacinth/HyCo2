...
if [ "$rec" = "rec" ];then
        ...
        logfile="${logfile}_rec"
fi
mkdir -p ${logfile}


CUDA_VISIBLE_DEVICES=${gpu} python -m src.eval.run_eval \
        --data ${data} \
        --save_dir ${logfile} \
